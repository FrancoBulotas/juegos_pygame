...
class NivelDos:
    ...
    def desarrollo(self, mouse_pos, nivel_dos):
        ...
        if not self.juego_en_pausa and not self.nivel_terminado:
            ...
            self.menu_nivel.dibujar_barra_superior(self.cronometro, self.nivel_dos)    
        ...
        if self.nivel_terminado:
            if not self.flag_archivo_guardado:
                self.archivo_puntos = guardar_archivo(nivel_dos.contador_puntos, nivel_dos=True)
                self.flag_archivo_guardado = True

            self.menu_nivel.dibujar_menu_fin(self.nivel_dos)
            self.eleccion_menu_fin(mouse_pos)
            # Esto es para que se pueda volver a jugar dandole a volver a jugar
            self.nivel_dos.ingreso_nivel_dos = True
