...
class Misil(pygame.sprite.Sprite):
    def __init__(self, bala_alien = False, bala_violeta=False, bala_plateado=False, alien_x = None, alien_y = None):
        super().__init__()
        self.bala_alien = bala_alien
        if bala_alien:
            if bala_violeta:
                self.image = pygame.image.load(RECURSOS + "enemigos\\alien-verde.png")   
                self.image = pygame.transform.scale(self.image, (60, 45))
            if bala_plateado:
                self.image = pygame.image.load(RECURSOS + "enemigos\\alien-plateado.png")   
