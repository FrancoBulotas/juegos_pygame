...
class NivelUno:
    ...
    def desarrollo(self, mouse_pos, nivel_uno) -> bool:
        ...
        if not self.juego_en_pausa and not self.nivel_terminado:
            ...
            self.menu_nivel.dibujar_barra_superior(self.cronometro, self.nivel_uno)    
        ...
        if self.nivel_terminado:
            if not self.flag_archivo_guardado:
                self.archivo_puntos = guardar_archivo(nivel_uno.contador_puntos, nivel_uno=True)
                self.flag_archivo_guardado = True

            self.menu_nivel.dibujar_menu_fin(self.nivel_uno)
            self.eleccion_menu_fin(mouse_pos)
            # Esto es para que se pueda volver a jugar dandole a volver a jugar
            self.nivel_uno.ingreso_nivel_uno = True
